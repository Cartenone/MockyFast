...
def workdir(tmp_path, monkeypatch):
    """Run a CLI command inside an empty directory.

    Uses pytest rather than CliRunner.isolated_filesystem, which click removed
    in 8.4.
    """
...
def write_users_json(directory: Path) -> Path:
    data_dir = directory / "data"
    data_dir.mkdir(exist_ok=True)

    data_file = data_dir / "users.json"
    data_file.write_text('[{"id": 1, "name": "Mario"}]', encoding="utf-8")
...
def test_init_creates_config_file(workdir):
    result = runner.invoke(app, ["init"])

    assert result.exit_code == 0
    assert "Sample file created" in result.output
    assert (workdir / "mockyfast.yaml").exists()


def test_init_fails_if_file_already_exists(workdir):
    (workdir / "mockyfast.yaml").write_text("already exists", encoding="utf-8")
...
def test_init_from_data_generates_a_resource_config(workdir):
    write_users_json(workdir)

    result = runner.invoke(app, ["init", "--from-data", "./data"])

    assert result.exit_code == 0

    generated = (workdir / "mockyfast.yaml").read_text(encoding="utf-8")
    assert "resources:" in generated
    assert "name: users" in generated

    # Paths must resolve from where the config was written, not from the data
    # folder it was derived from.
    assert "./data/users.json" in generated

    assert runner.invoke(app, ["validate", "mockyfast.yaml"]).exit_code == 0


def test_init_from_a_missing_data_path_fails(workdir):
    result = runner.invoke(app, ["init", "--from-data", "./nope"])

    assert result.exit_code == 1
    assert "Cannot read data path" in result.output
...
def test_validate_succeeds_for_valid_config(workdir):
...
def test_validate_fails_for_invalid_config(workdir):
    (workdir / "invalid.yaml").write_text("name: example\n", encoding="utf-8")
...
def test_validate_accepts_a_data_folder_without_any_yaml(workdir):
    write_users_json(workdir)
...
def test_validate_reports_shadowed_routes_as_warnings(workdir):
    (workdir / "mockyfast.yaml").write_text(
        """routes:
  - method: GET
    path: /users/{user_id}
...
""",
    ...
    assert "/users/me is unreachable" in result.output
    assert "Configuration is valid." in result.output
...
def test_create_app_from_env_reads_the_configured_path(tmp_path, monkeypatch):
    data_file = write_users_json(tmp_path)

    monkeypatch.setenv("MOCKYFAST_CONFIG", str(data_file.parent))
